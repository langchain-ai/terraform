...
- [x] **Sizing overlays were binary (ha/light) and didn't match AWS** — fixed: replaced with 4-tier `sizing_profile` variable (`minimum`, `dev`, `production`, `production-large`). `init-values.sh` copies the correct overlay from `examples/` based on `terraform.tfvars`; `deploy.sh` selects it automatically. `patch-lgp-resources.sh` added for LGP CR right-sizing on minimum/dev profiles.
