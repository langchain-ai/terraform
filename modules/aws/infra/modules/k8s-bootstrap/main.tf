...
#
# terraform_data only reruns its provisioner on replacement, not on a plain
# edit inside the command string — the path list itself is in
# triggers_replace so adding/removing a prefix here always re-applies.
resource "terraform_data" "fleet_stream_httproute" {
  count = var.enable_envoy_gateway && var.langsmith_domain != "" ? 1 : 0
  ...
  triggers_replace = [
    var.namespace,
    var.langsmith_domain,
    "/api/v1/fleet/threads,/api/v1/fleet/lg,/api-host/v2",
  ]
...
spec:
  ...
  - matches:
    - path:
        type: PathPrefix
        value: /api/v1/fleet/threads
    - path:
        type: PathPrefix
        value: /api/v1/fleet/lg
    - path:
        type: PathPrefix
        value: /api-host/v2
    backendRefs:
...
MANIFEST
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      export KUBECONFIG="$HOME/.kube/langsmith-${self.input.cluster_name}"
      aws eks update-kubeconfig --name ${self.input.cluster_name} --region ${self.input.region} --alias ${self.input.cluster_name} --kubeconfig "$KUBECONFIG" 2>/dev/null || true
      kubectl delete httproute langsmith-fleet-streams -n ${self.input.namespace} --ignore-not-found=true 2>/dev/null || true
    EOT
  }
