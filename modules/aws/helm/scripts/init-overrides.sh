#!/usr/bin/env bash
# init-overrides.sh — Generates langsmith-values-overrides.yaml from Terraform outputs.
#
# Usage (from aws/):
#   ./helm/scripts/init-overrides.sh
#
# Reads:
...
if [[ ! -f "$_tfvars" ]]; then
  ...
  echo "Run: cp $TF_DIR/terraform.tfvars.example $TF_DIR/terraform.tfvars" >&2
  exit 1
fi
...
_environment=$(grep -E '^\s*environment\s*=' "$_tfvars" 2>/dev/null \
  | sed 's/.*=[[:space:]]*"\(.*\)".*/\1/' | tr -d '[:space:]') || _environment="production"
_region=$(grep -E '^\s*region\s*=' "$_tfvars" 2>/dev/null \
  | sed 's/.*=[[:space:]]*"\(.*\)".*/\1/' | tr -d '[:space:]') || _region="${AWS_REGION:-us-east-2}"
_tls_source=$(grep -E '^\s*tls_certificate_source\s*=' "$_tfvars" 2>/dev/null \
  | sed 's/.*=[[:space:]]*"\(.*\)".*/\1/' | tr -d '[:space:]') || _tls_source="none"
...
IRSA_ROLE_ARN=$(terraform -chdir="$TF_DIR" output -raw langsmith_irsa_role_arn 2>/dev/null) || {
  echo "ERROR: Could not read langsmith_irsa_role_arn. Is 'terraform apply' complete?" >&2; exit 1
}
ALB_ARN=$(terraform -chdir="$TF_DIR" output -raw alb_arn 2>/dev/null) || ALB_ARN=""
ALB_DNS_NAME=$(terraform -chdir="$TF_DIR" output -raw alb_dns_name 2>/dev/null) || ALB_DNS_NAME=""
...
cat > "$OUT_FILE" << YAML
# Auto-generated by init-overrides.sh — do not edit auto-filled fields manually.
# Re-run init-overrides.sh to refresh Terraform outputs.
