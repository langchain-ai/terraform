...
# Insights — generate based on clickhouse_source
if [[ "$_enable_insights" == "true" ]]; then
  _insights_dst="$VALUES_DIR/langsmith-values-insights.yaml"
  if [[ "$_clickhouse_source" == "in-cluster" ]]; then
    cat > "$_insights_dst" << 'INSIGHTS'
# Auto-generated by init-values.sh — in-cluster ClickHouse.
# ClickHouse runs as a StatefulSet pod managed by the Helm chart (dev/POC only).
# For production use LangChain Managed ClickHouse:
#   set clickhouse_source = "external" in terraform.tfvars and re-run init-values.sh
config:
  insights:
    enabled: true
INSIGHTS
    pass "Generated: langsmith-values-insights.yaml (in-cluster ClickHouse)"
  else
