...
| smithcache | Standard_L16s_v3 | 16 | 128 GB | 0 | 3 | SmithDB local cache — only with `enable_smithdb = true` |
...
>
> The two SmithDB pools are tainted, so nothing else schedules onto them: `smithdb-local/instance-store=true:NoSchedule` on `smithcache` and `smithdb-local/compute=true:NoSchedule` on `smithcompute`. The matching node labels are what the chart's SmithDB overlay selects on. `Standard_L16s_v3` is the cache default because it carries a large local NVMe temporary disk — SmithDB uses that disk for its cache, so a VM size without a suitable temporary disk defeats the design even when AKS accepts the pool. Both pools scale from zero. The names are reserved: `additional_node_pools` cannot define `smithcache` or `smithcompute` while `enable_smithdb = true`.
