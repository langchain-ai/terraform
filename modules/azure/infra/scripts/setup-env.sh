...
#
# Two secret classes, handled differently:
#   User-provided (Postgres password, license key, admin password, admin email):
#     prompted on every run unless the matching env var is already set
#     (LANGSMITH_PG_PASSWORD / LANGSMITH_LICENSE_KEY / LANGSMITH_ADMIN_PASSWORD /
#     LANGSMITH_ADMIN_EMAIL). These are never read back from Key Vault.
#   Generated (API key salt, JWT secret, and the four encryption keys):
#     generated and cached locally on first run; Terraform stores them in Key Vault
#     on apply, and subsequent runs read them back from Key Vault (no prompt).
#
...
# Re-run ./setup-env.sh to regenerate this file.
