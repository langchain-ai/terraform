...
#   infra/secrets.auto.tfvars       — Terraform inputs written by setup-env.sh
#   infra/.pg_password              — fallback for a generated Postgres password
#   infra/.api_key_salt etc.        — legacy app-secret files; setup-env.sh no
#                                     longer writes these, but older deployments
#                                     still have them on disk
...
# The wizard's backup of the old config. It carries the same deployment details
# as terraform.tfvars, so leaving it behind leaves the directory unclean.
