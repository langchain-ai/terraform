...
| `aks-nodes` subnet, `/22` | Azure CNI is flat, so nodes and pods share it. `infra/main.tf:340` rejects anything under `(max_count + 1) × (max_pods + 1)`, which is 764 at the module's defaults |
...
| `postgres` subnet, delegated | Flexible Server is subnet-injected. Enforced at `infra/main.tf:307` |
...
| AKS cluster, OIDC + workload identity on | Federated credentials for the LangSmith service accounts. Read off the live cluster at `k8s-cluster/main.tf:135` |
...
- The module creates three role assignments unconditionally
  (`keyvault/main.tf:75`, `keyvault/main.tf:87`, `storage/main.tf:58`), and
