...
The `values.yaml` base and the `examples/` templates are **tracked in git**. The live `values-overrides.yaml` and the copied `langsmith-values-*.yaml` overlays are **gitignored** (generated, or contain deployment-specific values); `make init-values` generates them from the `examples/` templates and Terraform outputs.
...
The Azure-specific defaults that apply on every deploy, via both the shell path (`deploy.sh`) and the Terraform app path (`app/main.tf`). Sets NGINX as the default ingress class, enables the Azure Blob Storage engine, and sets the auth type and config secret name. You should not need to edit this file — environment-specific values (hostname, storage account, Workload Identity client ID, ingress/TLS, external secret references) go in `values-overrides.yaml`.
...
│   └── values/
│       ├── values.yaml                              # Azure base (NGINX ingress, Blob engine, auth defaults) — tracked
